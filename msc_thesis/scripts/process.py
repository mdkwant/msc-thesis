...
def remove_filter(slc):
    ...
    z2 = 1024-next(i for i,v in enumerate(sl2) if v > 0.4)
    bd = sl[z1+1:z2-1]
    
    data_fit = fit_cos(bd)
    bd_fit = np.zeros(1024)
    ...
    fit_2d = np.tile(bd_fit, (1024,1)).transpose()
    slc_1 = slc_f.copy()
    
    
    slc_phase = np.angle(slc_1)
    # + 1j * fit_2d * np.sin(slc_phase)
    slc_n = np.zeros([1024,1024], dtype=np.complex64)
...
def apply_window(slc_n, z1, z2, look):
    ...
    window = hamming(n, db) 
    x[start:end]=window
    window_2d = np.tile(x, (1024,1)).transpose()
...
def cross_spec(f1, f2, meta):
    ...
    im = im[y1:y2, x1:x2]
    rl = rl[y1:y2, x1:x2]
    axtick = [kx[x1], kx[x2], ky[y2], ky[y1]] #switch y1 and y2 for plot
    return im, rl, axtick, cspec1
...
def cmask(index,radius,array):
    ...
    mask = (x*x)  + y*y / (6**2) <= radius*radius
    array[mask] = 0
    return(array)
...
def SAR_1D(slc, buoy_spec, meta, location, impath, datestr, az, ra, thetasar=0):
    ...
    im13, rl13, axtick, cspec13 = cross_spec(f1, f3, meta)
    # process 2D spectra
    array_in = smooth(rl12, 9, window='hamming')
    ...
    def plot_SAR2D(n, f, f2, title, cm, vmi, vma, heading=-meta['heading']):
        ...
        #ax1.invert_yaxis()
        print("location of the SAR plot was "+ str(az)+" " + str(ra))
...
if __name__ == '__main__':
    ...
    datadir = 'd:\data\processed\python'
    dfile = 'S1A_IW_SLC__1SDV_20141125T183457'
    date = '20141125T183457'
    swath = 'IW2'
    ...
    meta, burst = preprocessing.read_XML(loc, swath)
    
    #pp = pprint.PrettyPrinter()
    
    saveloc = 'd:\data\processed\python\S1A_IW_SLC__1SDV_20141125T183457\IW2'
    
    #slc, var, cspec3, cspec2, cspec1 = multi_process(saveloc, meta, swath)
    loc = saveloc
    
    
    
    #rl_n = multi_process(loc, meta, swath, size=1024)
    
    multi_plot(rl_n, loc, meta, dfile, swath)
